...
pub trait Counters {
    ...
    /// Get many counters values at once, ar all if filter is None.
    fn values(filter: Option<CountersFilter>) -> RefVec<'i, CounterValue>;
    /// Get many counters values (or all if filter is None) in a compressed form to save bandwidth. Optional.
    fn values_compressed(filter: Option<CountersFilter>) -> RefVec<'i, u8>;
